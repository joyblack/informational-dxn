...
public class PositionController {
    ...
    public JoyResult add(@RequestBody @Valid PositionEntity position, BindingResult bindingResult) {
    ...
    @PostMapping(
            value = "/update",
            produces = {"application/json;charset=UTF-8"})
    public JoyResult update(@RequestBody @Valid PositionEntity positionEntity, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return JoyResult.buildFailedResult(Notice.REQUEST_PARAMETER_IS_ERROR, bindingResult.getFieldError().getDefaultMessage());
        } else {
            // copy
            return positionService.update(positionEntity);
        }
    }
