...
|openBank|[string]|开户行||
|entryTime|[number]|入职时间|入职时间，已格式化为yyyy-MM-dd|
...
|reviewTime|[date]|审核时间|审核人审核的时间，已格式化为yyyy-MM-dd|
|reviewRemark|[string]|备注信息|备注信息，一般审核不通过的原因可以从此字段中获取得到。|

## 3.3、员工离职信息（staff_desert）
|参数|类型|简介|备注|
|-|-|-|-|
|leaveTime|[date]|离职时间|离职的时间，已格式化为yyyy-MM-dd|
...
|backMiningMode|[enum]|回采方式：  BLASTING("炮采"),HIGH_GRADE("高档普采"),INTEGRATION_MACHINE("综合化机械")|

## 4.3、钻孔工作面(back-mining-face)
|参数名|类型|必填|说明|
|-|-|-|-|
|backMiningFaceName|[string]|名称|
|slopeLength|[decimal]|采面斜长(m)|
...
|drillCategory|[enum]|钻孔类别：THROUGH_LAYER("穿层钻孔"),BEDDING("穿层钻孔"),HIGH_POSITION("高位钻孔"),GEOLOGY("地质钻孔"),OTHER("其他钻孔")|
|drillType|[enum]|钻孔类型：GAS("瓦斯"),GEOLOGY("地质"),WATER_DETECTION("探水"),OTHER("其他")|
|drillRockCharacter|[enum]|钻孔岩性：COAL_LAYER("煤层"),ROCK_LAYER("岩层")|
|drillHole|[array(object)]|钻孔工作详情，是一个钻孔数组对象，参考：**4.4、钻孔详情**|
...
|参数名|类型|必填|说明|
|-|-|-|-|-|
|drillWork|[object]|是|关联的钻孔工作信息|
|dailyTime|[date]|是|日报日期|
|shifts|[enum]|是|班次|
|drillTeam|[object]|打孔队伍，其实就是部门信息，参考**2.4、部门**|
|peopleNumber|[number]|打孔的人数|
