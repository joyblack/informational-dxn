...
## 10、定时任务
* 安全巡检：每天凌晨零点会进行一次查验，未整改的条目若超过了截止时间，则会设置超时时间为已超时。
...
* 变更整改状态：从已整改到未整改如何处理？(不允许反向修改，已整改的不允许修改为违章该)
* 培训处需要选择平台信息，可能需要在部门表添加公司字段才能解决这个查询问题。[培训管理]的【培训煤矿】搜索项，是指发起搜索的，还是受搜索的；（统一添加一个煤矿平台作为参数字段）
